...
def compare_optimization_algorithms(data, target_categories):
	...
	for house in range(len(target_categories)):
		...
		theta, accuracy, accuracy_list, loss_list, epoch_list = classifier.fit_(x, y_labelled)
		classifier = MyLR(np.random.rand(x.shape[1] + 1, 1), 1e-1, 1000, None, 0.0, 'sgd')
		#classifier.optimizer = 'sgd'
		theta_sgd, accuracy_sgd, accuracy_list_sgd, loss_list_sgd, epoch_list_sgd = classifier.stochastic_fit(x, y_labelled)
		classifier = MyLR(np.random.rand(x.shape[1] + 1, 1), 1e-1, 1000, None, 0.0, 'mini')
		#classifier.optimizer = 'mini'
		theta_mini, accuracy_mini, accuracy_list_mini, loss_list_mini, epoch_list_mini = classifier.mini_batch_fit(x, y_labelled)
