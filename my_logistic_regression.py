...
class MyLogisticRegression():
    ...
    supported_optimizers = ['batch', 'mini', 'sgd']

    def __init__(self, thetas, alpha=0.001, max_iter=1000, penalty='l2', lambda_=1.0, optimizer='batch'):
        if self.check_validation(thetas, alpha, max_iter, penalty, lambda_, optimizer):
            self.alpha = alpha
    ...
    @staticmethod
    def check_validation(thetas, alpha, max_iter, penalty, lambda_, optimizer):
    ...
    def gradient(x, y, theta, lambda_=0.0):
        ...
        return gradient / x.shape[0], y_hat
    ...
    def fit_(self, x, y):
        ...
        if self.thetas.ndim == 1 and self.thetas.size == x.shape[1] + 1:
            ...
            print(
                f"Invalid input: wrong shape of {self.thetas}", self.thetas.shape)
            return None
        ...
        for i in range(self.max_iter):
            ...
            if self.penalty == None:
                grad, y_pred = self.gradient(temp_x, temp_y, new_theta)
            elif self.penalty == 'l2':
                grad, y_pred = self.gradient(
                    temp_x, temp_y, new_theta, self.lambda_)
...
# Handle invalid values in the gradient
        ...
        return self.thetas, accuracy, accuracy_list, loss_list, epoch_list
    ...
    def stochastic_fit(self, x, y):
        ...
        # Handle invalid values in the gradient
            ...
            loss = self.loss_(sample_y, y_pred)
            if i % 10 == 0:
                accuracy_list.append(accuracy)
                loss_list.append(loss)
                epoch_list.append(i)
